...
app.get('/health', async (req, res) => {
  ...
  const status =
    dbState === 'connected' && redisCheck !== 'error' && redisCheck !== 'Redis disabled'
      ? 'ok'
      : 'degraded';
...
app.get('/sitemap.xml', async (req, res) => {
  try {
    ...
    for (const cat of categories) {
      ...
      urls.push(
        `<url>
                    ...
                    ${
                      cat.image
                        ? `<image:image><image:loc>${escapeXml(cat.image)}</image:loc></image:image>`
                        : ''
                    }
                </url>`
      );
    ...
    for (const sub of subCategories) {
      const parentCat = categories.find((c) => String(c._id) === String(sub.category[0]));
      if (!parentCat) continue;
      const subUrl = `${baseUrl}/${valideURLConvert(parentCat.name)}-${parentCat._id}/${valideURLConvert(
        sub.name
      )}-${sub._id}`;
      urls.push(
        `<url>
                    ...
                    ${
                      sub.image
                        ? `<image:image><image:loc>${escapeXml(sub.image)}</image:loc></image:image>`
                        : ''
                    }
                </url>`
      );
    ...
    for (const blog of blogs) {
      ...
      urls.push(
        `<url>
                    ...
                    ${
                      blog.coverImage
                        ? `<image:image><image:loc>${escapeXml(blog.coverImage)}</image:loc></image:image>`
                        : ''
                    }
                </url>`
      );
