...
const RECEIPT_PRIVATE_KEY = process.env.RECEIPT_PRIVATE_KEY
  ...
  : "";
const RECEIPT_SIGNATURE_ALGORITHM = "RSA-PSS-SHA256";
...
function normalizeProducts(orderDoc = {}) {
  ...
  products.forEach((item, index) => {
    ...
    const unitPrice =
      Number(item.unitPrice ?? item.price ?? item.product_details?.price ?? 0) ||
      0;
    const total =
    ...
    items.push({
      ...
      name:
        item.product_details?.name ?? item.name ?? `Item ${index + 1}`,
      quantity,
      unitPrice,
      total,
      sku,
...
function buildReceiptPayload(orderDoc = {}, { integrityToken } = {}) {
  ...
  const totalAmount =
    Number(orderDoc.totalAmt ?? orderDoc.totalAmount ?? orderDoc.subTotalAmt ?? 0) ||
    0;
...
function sanitizeGuestContact(addressData = {}) {
  ...
  const normalizedEmail = emailRaw ? emailRaw.toString().trim().toLowerCase() : "";
  return {
    name: addressData.name?.toString().trim() ?? "",
...
async function resolveGuestOrderProducts(listItems = []) {
  ...
  listItems.forEach((item, index) => {
    ...
    const productDoc =
      typeof productInput === "object" && productInput?._id ? productInput : null;
    const productId =
      productDoc?._id ??
    ...
    if (!productDoc && !productId) {
      throw new Error(`Line item at position ${index + 1} is missing product information.`);
    }
...
export async function ensureIntegrityProof(orderDoc) {
  ...
  return {
    ...
    currency: orderDoc.currency ?? DEFAULT_CURRENCY,
    signatureGeneratedAt:
      updatedFields.signatureGeneratedAt ?? orderDoc.signatureGeneratedAt ?? null,
  };
}
...
function buildReceiptDownloadResponse(orderDoc) {
  ...
  return {
    ...
    signatureGeneratedAt: orderDoc.signatureGeneratedAt
      ...
      : new Date(orderDoc.updatedAt ?? orderDoc.createdAt ?? Date.now()).toISOString(),
    integrityToken: orderDoc.integrityToken ?? "",
...
export async function CashOnDeliveryOrderController(request, response) {
  try {
    ...
    const { list_items, totalAmt, addressId, subTotalAmt, metadata } = request.body;
    ...
    const computedTotal = products.reduce((sum, item) => sum + (item.total || 0), 0);
...
export async function GuestCashOnDeliveryOrderController(request, response) {
  try {
    const { list_items, totalAmt, subTotalAmt, isGuestOrder, metadata, ...addressData } =
      request.body;
    ...
    const contactInfo = sanitizeGuestContact(addressData);
    if (!contactInfo.customer_email) {
      return response.status(400).json({
